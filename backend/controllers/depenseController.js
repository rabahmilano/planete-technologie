...
export const addNewDepense = [
  ...
  async (req, res) => {
    ...
    const { montant, cpt, nature, dateDepense, observation, voyage_id } =
      req.body;

    try {
      const newDepense = await prisma.$transaction(async (tx) => {
        if (voyage_id) {
          const voyageInfo = await tx.voyage.findUnique({
            where: { id_voyage: parseInt(voyage_id, 10) },
            select: { statut_voy: true },
          });
          if (!voyageInfo) throw new Error("VOYAGE_NOT_FOUND");
          if (voyageInfo.statut_voy === "CLOTURE")
            throw new Error("VOYAGE_CLOTURE");
        }

        const natureInfo = await tx.nature_dep.findUnique({
          where: { id_nat_dep: parseInt(nature, 10) },
        });
        if (!natureInfo) throw new Error("NATURE_NOT_FOUND");
        ...
        const depenseCree = await tx.depense.create({
          data: {
            ...
            voyage_id: voyage_id ? parseInt(voyage_id, 10) : null,
            isAnnule: false,
          },
        });
      ...
      if (error.message === "VOYAGE_NOT_FOUND")
        return res
          .status(404)
      ...
      if (error.message === "NATURE_NOT_FOUND")
        return res
          .status(404)
          .json({ error: { message: "Nature de dépense introuvable." } });
...
export const updateDepense = [
  ...
  async (req, res) => {
    ...
    try {
      const oldDep = await prisma.depense.findUnique({
      ...
      if (!oldDep)
        return res
          .status(404)
          .json({ error: { message: "Dépense introuvable." } });
      if (oldDep.isAnnule)
        return res.status(403).json({
          error: { message: "Impossible de modifier une dépense annulée." },
        });
      ...
      res
        .status(200)
        .json({ message: "Dépense mise à jour avec succès (Reclassement)." });
    } catch (error) {
      res
        .status(500)
        .json({ error: { code: error.code, message: error.message } });
...
export const deleteDepense = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        message: "Dépense annulée avec succès. L'argent a été restitué.",
      });
    } catch (error) {
      if (error.message === "NOT_FOUND")
        return res
          .status(404)
          .json({ error: { message: "Dépense introuvable." } });
      if (error.message === "ALREADY_ANNULE")
        return res
          .status(403)
          .json({ error: { message: "Cette dépense est déjà annulée." } });
      res
        .status(500)
        .json({ error: { code: error.code, message: error.message } });
    }
